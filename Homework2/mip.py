...
class Verifier:
    ...
    def create_inputs(self, x_test, perturbation):
        ...
        for i in range(len(x_test)):
            ...
            # YOUR CODE HERE
            l_bound = x_test[i] - perturbation
            u_bound = x_test[i] + perturbation
            var = self.gurobi_model.addVar(lb=l_bound, ub = u_bound, name=f'x{i}')
    ...
    def add_linear_layer(self, input_vars, weight, bias):
        ...
        for i in range(output_size):
            input_vars = list(input_vars)  # Convert dict_values to list
            expr = sum(weight[i, j] * input_vars[j] for j in range(input_size)) + bias[i]
    ...
    def solve_objectives(self, objectives, direction='minimization'):
        ...
        for i in range(len(objectives)):
            ...
            if direction == 'minimization':
                self.gurobi_model.setObjective(objectives[i], sense=gurobipy.GRB.MINIMIZE)
                # YOUR CODE HERE
                # self.gurobi_model.setObjective(...)
                
            elif direction == 'maximization':
                # YOUR CODE HERE
                self.gurobi_model.setObjective(objectives[i], sense=gurobipy.GRB.MAXIMIZE)
                
            else:
                raise ValueError(direction)
            self.gurobi_model.optimize()
            #assert self.gurobi_model.optimize()
            ...
            if self.gurobi_model.status == gurobipy.GRB.OPTIMAL:
                print(f"Optimization successful for objective {i}.")
            elif self.gurobi_model.status == gurobipy.GRB.INFEASIBLE:
                print(f"Model is infeasible for objective {i}.")
                optimal_objs.append(None)
                optimal_vars.append(None)
                return
            elif self.gurobi_model.status == gurobipy.GRB.TIME_LIMIT:
                print(f"Time limit exceeded for objective {i}.")
                optimal_objs.append(None)
                optimal_vars.append(None)
                return
            else:
                print(f"Optimization failed for objective {i} with status {self.gurobi_model.status}.")
                optimal_objs.append(None)
                optimal_vars.append(None)
                return
